...
async function fetchProjects(token?: string): Promise<ProjectWithProgress[]> {
  const projects = await backendApi.get<Array<{
    id: string;
    name: string;
    description: string;
    status: string;
  ...
  const [tasks, members] = await Promise.all([
    backendApi.get<Array<{ project_id: string; status: string; assignee_id: string | null }>>("/api/tasks", token),
    backendApi.get<Array<{ project_id: string; user_id: string }>>("/api/project-members", token),
  ]);

  const tasksByProject = new Map<string, Array<{ project_id: string; status: string; assignee_id: string | null }>>();
  ...
  for (const member of members ?? []) {
    ...
    if (projectMembers) projectMembers.push(member.user_id);
    else membersByProject.set(member.project_id, [member.user_id]);
  }
...
function DashboardPage() {
  ...
  const pendingQ = useQuery({
    ...
    queryFn: async (): Promise<PendingTask[]> => {
      const rows = await backendApi.get<Array<{
        id: string; title: string; status: string; deadline: string | null;
        project_id: string; projects: { name: string } | null;
      }>>(`/api/tasks?assignee_id=${user!.id}&statuses=NOT_STARTED,ACTIVE`, session?.access_token);
      return rows
        .map((r) => ({
          id: r.id,
          title: r.title,
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        <h2 className="font-display text-ssf uppesemirocd uppercase tracking-[0.18em]em] t-mutedext-muted-found">
          Projects
        </h2>
        {isLoading ? (
...
function NewProjectDialog({ profiles, userId }: { profiles: Profile[]; userId: string }) {
  ...
  const create = useMutation({
    mutationFn: async () => {
      await backendApi.post<{ id: string }>(
        "/api/projects",
